...
if __name__ == "__main__":
    ...
    start_dt = datetime.datetime(2013, 3, 1)
    end_dt = datetime.datetime(2021, 3, 1)
    ...
    #    price_AGG = pdr.get_data_yahoo('AGG', start=start_dt, end=end_dt)
        #
        Ticker_AllAsset = ['VCIT','BNDX','GLD','QQQ','VGT','AGG']
        #RESULTS FROM HIERARCHICAL RISK PARITY ['AGG','BNDX','GLD','VCIT']
        #ALL ASSETS ['VOO', 'QQQ','VEA','BNDX','VCIT','LQD','AGG','BSV','GLD','USO']
    ...
    datestr = ret_assets.index[ret_assets.index >= '2013-03-01']
    # previous month
    ...
    sharpe_table1 = pd.Series(OrderedDict((('risk_parity', sharpe_ratio_riskP.values),
                     ...
                     ('BNDX',sharpe_ratio_assets['BNDX']),
                     ('VGT', sharpe_ratio_assets['VGT']),
                     ('QQQ', sharpe_ratio_assets['QQQ']),
                     ('AGG', sharpe_ratio_assets['AGG'])
                     )))
    ...
    FAMA5 = pd.read_csv('F-F_Research_Data_5_Factors_daily.csv',names=['Date','Mkt-RF','SMB','HML','RMW','CMA','RF'],skiprows=13084, nrows=1914,index_col=('Date'))
    print(FAMA5.iloc[:10])

    #2015-06-23 13085
    #2023-01-31 15000

    X = sm.add_constant(FAMA5).drop(['RF'],axis=1)
    RF = pd.DataFrame(FAMA5['RF'])
    print(FAMA5['RF'].iloc[:10])
    #Add a constant to the independent value

    df = Nret.join(RF).dropna()
    excess_ret = df.iloc[:,0] - df.iloc[:,1]
    y = excess_ret
    ...
    print (lreg.intercept_, lreg.coef_, mse)
...
# Tickers of assets
tickers =  ['VCIT','BNDX','GLD','QQQ','VGT','AGG']
...
#pie chart
ax = rp.plot_pie(w=w2, title='Portfolio', height=6, width=10,
                 cmap="tab20", ax=None)